c=======================================================================
c=======================================================================
      subroutine NORNIGNORNIGSAS(nburn,nskip,nsave,ndata,ydata,
     & alphas,betas,c0,d0,mu0s,k0s,a0s,b0s,mctclusters,mcnblocks,
     & mcassocg,mcsasweight,independent)
c=======================================================================
c=======================================================================
c     BEGIN: NORNIGNORNIGSAS SUBROUTINE
      implicit none
c=======================================================================
c     DESCRIPTION
c=======================================================================
c     "NORNIGNORNIGSAS" RETURNS POSTERIOR SAMPLES FROM THE FOLLOWING
...
c
c     A.3) G|W~{W*DELTA[0]}+{(1-W)*UNIFORM([-L[B],U[B]]-{0})}.
...
c
c     A.4) W~BETA(C[0],D[0]).
c
c        · SHAPE PARAMETERS C[0] AND D[0] ARE FIXED.
c
...
c          · DF(·): DATA FACTOR.
c
c     C) DATA FACTOR MODEL: FOR ALL S IN {T[J-1,L]+1,...,T[J,L]},
c        J IN {1,...,B[L]} AND L IN {1,2},
c
c        Y[S,L]|MU[J,L],SIGMA[J,L]^2~NORMAL(MU[J,L],SIGMA[J,L]^2).
c
c        HERE, OBSERVATIONS ARE INDEPENDENT GIVEN MU[J,L] AND
c        SIGMA[J,L]^2.
c
c     C.1) MU[J,L]|SIGMA[J,L]^2~NORMAL(MU[0,L],(SIGMA[J,L]^2)/K[0,L]).
c
c          · MEAN MU[0,L] AND PRECISION K[0,L] ARE FIXED.
c
c     C.2) SIGMA[J,L]^2~INVERSE-GAMMA(A[0,L],B[0,L]).
c
c          · SHAPE A[0,L] AND SCALE B[0,L] ARE FIXED.
c
c     - SEE "LOGDFNORNIG" SUBROUTINE IN FILE "TOOLSDF.F" FOR MORE
c       DETAILS ABOUT DF(Y[S,L]:S=T[J-1,L]+1,...,T[J,L]).
c=======================================================================
c     INPUTS: MCMC
c=======================================================================
...
c     independent: Binary to indicate if time series are independent
      integer independent
c=======================================================================
...
c     d0: SHAPE (D[0])
      real*8 d0
c     mu0s: MEANS (MU[0,L]:L=1,2)
      real*8 mu0s(2)
c     k0s: PRECISIONS (K[0,L]:L=1,2)
      real*8 k0s(2)
c     a0s: SHAPES (A[0,L]:L=1,2)
      real*8 a0s(2)
c     b0s: SCALES (B[0,L]:L=1,2)
      real*8 b0s(2)
c=======================================================================
...
c     mcsasweight: SPIKE-AND-SLAB WEIGHT
      real*8 mcsasweight(nsave)
c=======================================================================
c     C++ FUNCTIONS
c=======================================================================
      real*8 betar
...
c=======================================================================
         call rassocgsas(ndata,tclusters,alphas,betas,w,rw)
         g=rw
c=======================================================================
c        UPDATING SPIKE-AND-SLAB WEIGHT (W)
         call rweightbet(g,c0,d0,rw)
         w=rw
...
c           ASSOCIATION PARAMETER
            mcassocg(stored)=g
c           SPIKE-AND-SLAB WEIGHT
            mcsasweight(stored)=w
c=======================================================================
...
c=======================================================================
      return
c     END: NORNIGNORNIGSAS SUBROUTINE
      end
c=======================================================================
c=======================================================================
